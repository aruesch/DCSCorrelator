...
int main()
{
	...
	mxArray				*corrMat = NULL;
	mxArray				*iter = NULL;
	mxArray				*delayMat = NULL;
	mxArray				*testVar = NULL;
	...
	//int dims2[1] = { FIRST_DIM };
	int bytes_to_copy = (256 * 8) * sizeof(float);
	double data[] = { 1,4,7,2,5,8,3,6,9,10,13,16,11,14,17,12,15,18 };
	
	ep = engOpen(NULL); //open MATLAB workspace
	engEvalString(ep, "addpath(genpath('C:\\Users\\user\\Desktop\\DCS_scripts\\Results'))");
	engEvalString(ep, "testMATLAB");
	...
	delayMat = mxCreateDoubleMatrix(256, 1, mxREAL);
	corrMat = mxCreateDoubleMatrix(256 * 8, 1, mxREAL);
	iter = mxCreateDoubleMatrix(1, 1, mxREAL);

	//testVar = mxCreateDoubleMatrix(18, 1, mxREAL);
	//pr_test = mxGetPr(testVar);
	//memcpy(pr_test, data, 18 * sizeof(double));
	//cout << "pointer-test " << *pr_test << "\n";
	//engPutVariable(ep, "testVar", testVar);
	//engEvalString(ep, "addpath(genpath('C:\\Users\\user\\Desktop\\DCS_scripts\\Results'))");
	//engEvalString(ep, "testMATLAB(testVar);");
	//mxDestroyArray(testVar);
	...
	pr = mxGetPr(corrMat);
	pr_iter = mxGetPr(iter);
	pr_delayTime = mxGetPr(delayMat);
	...
	// Copying delay Time to pointer delay time
	memcpy(pr_delayTime, DelayTime, 256 * sizeof(double));	
	engPutVariable(ep,"delayTime", delayMat);
	...
	if (hDLL != NULL) {
		...
		for (k = 0; k<10; k++)
		{
			...
			memcpy(pr, corr, 256 * 8 * sizeof(float)); 
			

			engPutVariable(ep, "corrMat", corrMat);
			engPutVariable(ep, "iter", iter);

			

			engEvalString(ep, "addpath(genpath('C:\\Users\\user\\Desktop\\DCS_scripts\\Results'))");
			engEvalString(ep, "plotCorr(corrMat, delayTime);");
	...
	//engClose(ep);
	mxDestroyArray(corrMat);
	mxDestroyArray(iter);
	mxDestroyArray(delayMat);
